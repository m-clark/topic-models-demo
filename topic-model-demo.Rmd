...
We begin the simulation by creating topic probabilities $\theta$. There will be 3 topics. Half of our documents will have probabilities of topics for them ($\theta_1$) which will be notably different from the other half ($\theta_2$).   We'll use the <span class="pack">DirichletReg</span> package for the <span class="func">rdirichlet</span> function, but it actually could be used to model topic probabilities via additional covariates if so desired.
...
Ndocs = 100                                        # Number of documents
WordsPerDoc = rpois(Ndocs, 100)                    # Avg. number of total words/terms in a document
thetaList = list(c(A=.6, B=.25, C=.15), c(A=.1, B=.1, C=.8))   # Topic proportions for first and second half of data
theta_1 = t(replicate(Ndocs/2, thetaList[[1]]))    # First half of documents show higher probability of topic '1' and '2'
theta_2 = t(replicate(Ndocs/2, thetaList[[2]]))    # Second half of documents show higher probability of topic '3'
theta = rbind(theta_1, theta_2)                    # Topic probabilities for 500 docs
```
...
B_k_1 = rdirichlet(n=length(cuts[[1]]), alpha=c(8,1,1))         # topics, i.e. distribution over terms
B_k_2 = rdirichlet(n=length(cuts[[2]]), alpha=c(1,8,1))         # topics, i.e. distribution over terms
B_k_3 = rdirichlet(n=length(cuts[[3]]), alpha=c(1,1,8))         # topics, i.e. distribution over terms
B_k = rbind(B_k_1, B_k_2, B_k_3)

# given topic assignment, draw words according to doc size
...
With a matrix approach, we don't need explicit topic label, just the topic indictator matrix and topic probabilities. Later on I provide some code that will let you see this for yourself that the results are similar, but we'll just stick to one result to keep things clear.
...
Let's look at the results. To be clear, the topic labels are *completely arbitrary*, so what is topic "1" for one analysis might be topic "3" for another. The main thing is the recovery of the distribution of the topics, which will be roughly `r theta_1[1,]` for the first half (arbitrary order), and `r theta_2[1,]` for the other. I show cho
